            ...
            <div className="bg-zinc-900 w-full max-w-md rounded-2xl shadow-2xl border border-zinc-800 overflow-hidden animate-slide-up flex flex-col max-h-[90vh]">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto flex-grow">
                    ...
                    <div>
                        ...
                        <input type="date" required value={date} onChange={(e) => setDate(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-800 rounded-lg text-zinc-200 outline-none focus:ring-2 focus:ring-terracotta-500 transition-all" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-zinc-400 mb-1">จำนวนเงิน (บาท)</label>
                        <input type="number" min="0" step="0.01" autoFocus required value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full text-4xl font-bold text-white bg-transparent border-b-2 border-zinc-800 focus:border-terracotta-500 outline-none py-2 placeholder-zinc-800 transition-colors" placeholder="0.00" />
                    </div>
                                ...
                                {Array.from(new Set([...(formType === 'INCOME' ? incomeCategories : expenseCategories), category].filter(Boolean))).map(c => (
                                    <div key={c} className="relative group">
                                        <button
                                            ...
                                            className={`w-full p-2 text-sm rounded-lg border transition-colors text-left truncate relative ${category === c && !isManagingCats ? 'bg-terracotta-600 border-terracotta-500 text-white font-bold shadow-lg shadow-terracotta-900/20' : 'bg-zinc-900 border-zinc-800 text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200'} ${isManagingCats ? 'border-dashed border-zinc-600' : ''}`}
                                        >
                                            {c}
                    ...
                    <div>
                        ...
                        <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className="w-full p-3 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-200 outline-none focus:ring-2 focus:ring-terracotta-500">
                    ...
                    {formType === 'TRANSFER' && (
                        <div>
                            ...
                            <select value={toAccount} onChange={(e) => setToAccount(e.target.value)} className="w-full p-3 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-200 outline-none focus:ring-2 focus:ring-terracotta-500">
                                {ACCOUNTS.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                            </select>
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        <input type="text" value={note} onChange={(e) => setNote(e.target.value)} className="w-full p-3 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-200 outline-none focus:ring-2 focus:ring-terracotta-500" placeholder="ระบุรายละเอียด..." />
                    </div>
                    ...
                    <div className="pt-2 flex gap-3">
                        ...
                        <button type="submit" disabled={!formBranchId || isSubmitting} className="flex-1 bg-terracotta-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-terracotta-500 transition-all flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-terracotta-900/20">
