    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-bold text-zinc-200">ประวัติรายการ {selectedBranchId !== 'HQ' && <span className="text-terracotta-400 text-sm font-normal">({currentBranchName})</span>}</h3>
                <div className="flex gap-2 relative">
                    {onImportTransactions && (
                        <button onClick={() => setShowImportModal(true)} className="text-terracotta-400 flex items-center gap-1 text-sm bg-zinc-900 border border-zinc-800 px-3 py-1 rounded-full hover:bg-zinc-800 transition-colors">
                    ...
                    {/* Filter Mode - Type */}
                    <button onClick={() => setShowFilterMenu(!showFilterMenu)} className={`flex items-center gap-1 text-sm bg-zinc-900 border border-zinc-800 px-3 py-1 rounded-full transition-colors ${filterType !== 'ALL' ? 'text-terracotta-400 border-terracotta-500/50' : 'text-zinc-400'}`}>
                        <Filter size={14} />
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`flex items-center gap-1 text-sm bg-zinc-900 border border-zinc-800 px-3 py-1 rounded-full transition-colors ${startDateFilter || endDateFilter ? 'text-terracotta-400 border-terracotta-500/50' : 'text-zinc-400'}`}
                        >
                            <Calendar size={14} />
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2 pt-2 border-t border-zinc-800">
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <input
                                                ...
                                                className="bg-black border border-zinc-800 rounded-lg p-2 text-xs text-white outline-none focus:ring-1 focus:ring-terracotta-500"
                                            />
                                        </div>
                                        <div className="flex flex-col gap-1">
                                    ...
                                    <div className="flex gap-2 pt-2">
                                        ...
                                        <button
                                            ...
                                            className="flex-1 text-xs py-2 bg-terracotta-600 hover:bg-terracotta-500 text-white rounded-lg font-bold"
                                        >
                                            ตกลง
                                        </button>
                                    </div>
                    ...
                    {showFilterMenu && (
                        <div className="absolute top-full right-0 mt-2 w-40 bg-zinc-900 border border-zinc-800 rounded-xl shadow-xl overflow-hidden z-20 animate-fade-in">
                            <button onClick={() => { setFilterType('ALL'); setShowFilterMenu(false); }} className={`w-full text-left px-4 py-2 text-sm hover:bg-zinc-800 ${filterType === 'ALL' ? 'text-terracotta-400 bg-terracotta-500/10' : 'text-zinc-300'}`}>ทั้งหมด</button>
            ...
            {filteredGroupedTransactions.length === 0 ? (
                <div className="p-12 text-center text-zinc-600 bg-zinc-900 rounded-xl border border-zinc-800 border-dashed">
                    ...
                    {(startDateFilter || endDateFilter || filterType !== 'ALL') && (
                        <button onClick={() => { clearDateFilter(); setFilterType('ALL'); }} className="mt-4 text-terracotta-400 text-sm hover:underline">ล้างการเลือกทั้งหมด</button>
                    )}
                </div>
            ) : (
                                    ...
                                    <div key={t.id} className="p-4 hover:bg-zinc-800/50 flex justify-between items-center transition-colors">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs text-zinc-500 flex flex-wrap items-center gap-2 mt-1">
                                                    {selectedBranchId === 'HQ' && <span className="text-terracotta-300 bg-terracotta-900/20 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border border-terracotta-500/20">{branches.find(b => b.id === t.branchId)?.name.split(' ')[1]}</span>}
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex gap-1">
                                                {canEdit(t) && (
                                                    <>
                                                        <button onClick={() => onEdit(t)} className="text-zinc-600 hover:text-terracotta-400 p-1"><Edit2 size={14} /></button>
